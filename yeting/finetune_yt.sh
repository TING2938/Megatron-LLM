...
if [[ $MODEL = falcon ]]; then
	...
	EXTRA_ARGS="--vocab_file=/root/models/llama-2-7b-chat-hf-megatron/tokenizer.model --use_rms_norm
	...
	if [[ $MODEL == llama ]]; then
		...
		EXTRA_ARGS="$EXTRA_ARGS --vocab_file=/root/models/llama-2-7b-chat-hf-megatron/tokenizer.model"
...
CUDA_VISIBLE_DEVICES="4,5" CUDA_DEVICE_MAX_CONNECTIONS=1 OMP_NUM_THREADS=16 torchrun $DISTRIBUTED_ARGS finetune.py \
